...
- **后台下载进度指示** - 在「设置 > 关于」页面实时显示后台下载进度条和百分比
- **后台下载安装按钮** - 后台下载完成后自动显示「立即安装」按钮，支持一键完成更新
- **更新通知后台选项** - 更新检测弹框中新增「后台下载」按钮，支持将更新下载任务切换到后台执行

### 优化

- **更新事件统一化** - 后台下载期间也会持续发送 `update-download-progress` 事件，确保 UI 状态同步
- **测试覆盖完善** - 为 GeneralSettings 组件新增 7 个测试用例，覆盖更新状态管理和后台下载流程
